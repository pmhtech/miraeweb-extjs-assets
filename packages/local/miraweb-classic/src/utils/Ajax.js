...
Ext.define('Miraeweb.utils.Ajax', {
    ...
    request : function (options) {
        ...
        if (confirmMsg) {
              Miraeweb.Msg.confirm(confirmMsg.title, confirmMsg.message, function (b) {
             me._runAjax(options);
             });
        } else {
            me._runAjax(options);
    ...
    defaultResponseSuccess: function(response,options){
...
/*
        ...
        switch(resObj.error){
            ...
            case 'JwtVerifyException' :
                Miraeweb.Cookie.removeCookie();
                location.href="";
                return;
            case 'JwtNotFoundException':
                Miraeweb.Ajax.refreshAccessToken(response);
                break;

            case 'UserIdNotFoundException':
                Miraeweb.Msg.alert('확인','사용자를 찾을수 없습니다.');
                return;
            case 'UserPasswordMismatchException':
                Miraeweb.Msg.alert('확인','비밀번호가 일치하지 않습니다.');
                return;
            default:
                ...
                Miraeweb.Msg.alert('확인',Ext.encode(resObj));
                return;
    ...
    _runAjax: function (options) {
        ...
        options.success  = function (response) {
            ...
            Miraeweb.Ajax.defaultResponseSuccess(response,options);
            var resObj = [];
            ...
            if(!Miraeweb.Ajax.defaultResponseFailure(resObj)){
                if(loadComp.isComponent===true){
            ...
            if (successMsg) {
                ...
                if(me.useToast===true){
                    ...
                    Miraeweb.Msg.alert(successMsg.title, successMsg.message, function () {
                        Ext.callback(callBackFunc, me, [resObj]);
                    });
                }
